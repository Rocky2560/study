...
//Explanation: The largest element of the array is 10 there is no second largest element.
    ...
    private static int secondLargestWay(int[] in) {
        ...
        for (int j = 0; j < length; j++) {
            if (in[j]< largest &&  in[j]< secondLargest && in[j]> thirdLargest) thirdLargest = in[j];
        }
        return thirdLargest;
    ...
    private static int secondLargestOtherWay(int[] in) {
        ...
        int largest =-1, secondLargest =-1, thirdLargest = -1;
        for (int i = 0; i < length; i++) {
            if (in[i]> largest ) {thirdLargest = secondLargest; secondLargest=largest; largest = in[i];}
            else if (in[i]< largest &&  in[i]> secondLargest){ thirdLargest = secondLargest; secondLargest = in[i];}
            else  if (in[i]< largest &&  in[i]< secondLargest && in[i]> thirdLargest) thirdLargest = in[i];
        }
        return thirdLargest;
